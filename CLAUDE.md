...
- `service/` — `GatewayAiClient` and `NewApiGatewayClient` are the chokepoint for outbound model calls; `GatewayConfigService` resolves the active gateway URL/key (DB row > env var, see below); `GatewayModelSyncService` pulls the gateway's model catalogue into the local `Model` table; `AssignmentService` handles assignment CRUD.
- `util/` — `ModelConfigUtil` builds model config JSON; `GatewayResponseUtil` parses gateway responses.
- `config/`
...
Datasource: [application.yml](backend/src/main/resources/application.yml) is configured for MySQL on localhost (`jincai_zhihui` schema, root user, empty password). H2 is on the classpath as a test/fallback dependency but the live config uses MySQL. The AI Gateway uses its own internal SQLite — that is separate from the application database.
...
Key student flow: `Workspace.vue` (chat-style generation at `/student/generate`) → `ClassGallery.vue` (browse class works) → `StudentAssignments.vue` → `AssignmentDetail.vue` / `AssignmentPlay.vue`. Key teacher flow: `Dashboard` → `Models` (sync from gateway) → `Students` (manage accounts) → `Assignments` (create/grade) → `Live` → `Settings` / `Templates`.
