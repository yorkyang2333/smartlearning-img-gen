...
export default function ModelsPage() {
  ...
  const { data: quotaRes } = useSWR('/api/quota', fetcher);
  const [quotaFormData, setQuotaFormData] = useState({ dailyLimit: 50, blockedWords: '' });
  ...
  useEffect(() => {
    if (quotaRes?.data) {
      setQuotaFormData({
        dailyLimit: quotaRes.data.dailyLimit || 50,
        blockedWords: quotaRes.data.blockedWords ? JSON.parse(quotaRes.data.blockedWords).join(', ') : ''
      });
    }
  }, [quotaRes]);
  ...
  const handleQuotaSubmit = async (e: React.FormEvent) => {
    ...
    setQuotaMessage('');
    try {
      const wordsArray = quotaFormData.blockedWords
        .split(/[，,]/)
        .map(w => w.trim())
        .filter(w => w.length > 0);
      ...
      const res = await fetch('/api/quota', {
        ...
        body: JSON.stringify({
          dailyLimit: quotaFormData.dailyLimit,
          blockedWords: wordsArray.length > 0 ? wordsArray : null
        })
      });

      if (res.ok) setQuotaMessage('设置保存成功');
      else setQuotaMessage('设置保存失败');
    } catch (err) {
      setQuotaMessage('发生错误');
    } finally {
            ...
            <form onSubmit={handleQuotaSubmit}>
              <div style={{ marginBottom: '2.5rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  ...
                  <input 
                    type="number" 
                    style={{ width: '100%', maxWidth: '300px', padding: '10px 12px', border: '1px solid var(--hairline)', borderRadius: '6px' }}
                    value={quotaFormData.dailyLimit} 
                    onChange={e => setQuotaFormData({...quotaFormData, dailyLimit: parseInt(e.target.value) || 0})} 
                    min="1"
                  />
              ...
              <div style={{ marginBottom: '2.5rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  ...
                  <textarea 
                    ...
                    rows={4} 
                    value={quotaFormData.blockedWords} 
                    onChange={e => setQuotaFormData({...quotaFormData, blockedWords: e.target.value})} 
