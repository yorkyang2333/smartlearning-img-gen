...
- **统一 AI 接入**：通过 OpenAI 兼容格式统一接入多家 AI 服务商，教师在管理页面配置 API 连接
...
| AI 接入 | OpenAI 兼容格式，支持任意兼容 API 平台 |
...
│       ├── service/      GatewayAiClient, NewApiGatewayClient, GatewayConfigService, AssignmentService
...
```
Vue 前端 (5173)  ──JWT──▶  Spring Boot (8080)  ──OpenAI 兼容格式──▶  AI API 平台  ──▶  上游服务商
```

后端所有 AI 调用统一走 OpenAI 兼容接口（`/v1/chat/completions`、`/v1/images/generations`），不直接对接任何服务商。教师在管理页面配置 API 连接地址和 Key。
